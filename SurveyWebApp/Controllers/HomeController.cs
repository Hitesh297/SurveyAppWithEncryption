...
namespace SurveyWebApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(SurveyContext db, ILogger<HomeController> logger, IHttpContextAccessor httpContextAccessor, IConfiguration config)
        {
            this._db = db;
            _logger = logger;
        ...
        public IActionResult Create(Survey ajaxSurvey)
        {
            ...
            if (survey != null)
            {
                ...
                EncryptQuestions(survey.Questions);
            }
            else
            {
                survey = new Survey();
        ...
        public IActionResult Edit(int? id)
        {
            ...
            if (id != null)
            {
                ...
                DecryptQuestions(survey.Questions);
        ...
        public IActionResult Edit(Survey FormSurvey)
        {
            ...
            if (surveyFromdb != null)
            {
                ...
                foreach (Question que in FormSurvey.Questions)
                {
                    ...
                    {
                        existingQues.QuestionText = EncryptionHelper.EncryptionHelper.EncryptString(_config["encryptionkey"], que.QuestionText);
                        foreach (var choice in que.Choices)
                        {
                            var existingChoice = existingQues.Choices.Where(x => x.Id == choice.Id).FirstOrDefault();
                            if (existingChoice != null)
                            {
                                existingChoice.ChoiceText = EncryptionHelper.EncryptionHelper.EncryptString(_config["encryptionkey"], choice.ChoiceText);
                            }
                        }
                    }
                }
            ...
            {
                EncryptQuestions(FormSurvey.Questions);
        ...
        public IActionResult Results(int surveyId)
        {
            ...
            if (surveyId != 0)
            {
                ...
                vm.Responses = srs;
                DecryptQuestions(vm.Survey.Questions);
        ...
        public IActionResult MainSurvey(int surveyId, string employeeId)
        {
            ...
            if (vm.Survey.PublishedOn == null || (vm.Survey.ExpiresOn < DateTime.Now))
            {
               return View("Expired");
            }
        ...
        public void EncryptQuestions(List<Question> questions)
        {
            foreach (var question in questions)
            {
                question.QuestionText = EncryptionHelper.EncryptionHelper.EncryptString(_config["encryptionkey"], question.QuestionText);
                foreach (var choice in question.Choices)
                {
                    choice.ChoiceText = EncryptionHelper.EncryptionHelper.EncryptString(_config["encryptionkey"], choice.ChoiceText);
                }
            }
        }

        public void DecryptQuestions(List<Question> questions)
        {
            foreach (var question in questions)
            {
                question.QuestionText = EncryptionHelper.EncryptionHelper.DecryptString(_config["encryptionkey"], question.QuestionText);

                foreach (var choice in question.Choices)
                {
                    choice.ChoiceText = EncryptionHelper.EncryptionHelper.DecryptString(_config["encryptionkey"], choice.ChoiceText);
                }
            }
        }
    }
